...
## B9. 生存层与 TCGA 层复算（本轮新增核查）
- **结论**：Cox 模型从随包分数重拟合，**68 个比较的 HR 最大绝对差 = 0**（含 ESCC 纯鳞癌 uni 与 age+stage）。
- **模块分数重建**：从基因级表达表独立重建患者级分数，秩相关 **0.95–0.98**；绝对值差异来自聚合顺序（加权/先 z 后平均），故已在 Methods 明确写出确切构造规则。
