#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v14 close-out: reviewer-response document, checksum tidy-up, ZIP rebuild, push."""
...
doc = """# GPT6 对 v13 意见的逐条处置（v13 → v14）
...
"""
open(os.path.join(V14,"05_审稿意见与处置_v14.md"),"w",encoding="utf-8").write(doc)
print("response document written")
# tidy the checksum file (drop blank line) and re-verify
sp=os.path.join(PK,"08_qc","checksums_sha256.txt")
txt=open(sp,encoding="utf-8").read().strip()
...
print("sha256sum:", (r.stdout.strip() or "all OK"))
# rebuild zips including the response document
z=os.path.join(r"C:\Users\fengq\Desktop","★Zenodo上传-就选这个_v14"); shutil.make_archive(z,"zip",PK)
stage=os.path.join(V14,"_stage2"); shutil.rmtree(stage,ignore_errors=True); os.makedirs(stage)
...
for a,b in [("01_manuscript/DataDescriptor_v14.md","manuscript/DataDescriptor_v14.md"),
            ("05_审稿意见与处置_v14.md","reports/v27_v14_response_to_gpt6_v13_review.md")]:
    sp2=os.path.join(V14,a)
...
for f in ["Fig1_resource_overview.png","Fig2_technical_validation.png","Fig3_comparability_checks.png","Fig4_sensitivity_checks.png"]:
    shutil.copy(os.path.join(V14,"02_figures",f), os.path.join(repo,"figures",f))
for f in os.listdir(PK):
    pass
import glob
for f in glob.glob(os.path.join(PK,"08_qc","qc_*.csv"))+glob.glob(os.path.join(PK,"04_modules","module_overlap_matrix.csv"))+glob.glob(os.path.join(PK,"01_cohort_registry","contrast_ontology.csv")):
...
for s in [["git","add","-A"],["git","-c","user.name=sushuqiong","-c","user.email=fengqi9666@qq.com","commit","-q","-m",
  "v14: four-figure set with pixel-level QA (no clipping/collisions), reference list, search-and-screening process, expression and single-cell QC, deterministic transformation rule, module overlap matrix, study-level overlap and dedup sensitivity, control-tissue ontology, survival data dictionary, independent marker-proxy composition method, prediction intervals and tolerance-based reproduction wording"],
