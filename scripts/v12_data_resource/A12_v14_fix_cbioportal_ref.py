#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v14: repair the one mis-resolved entry (the cBioPortal correction notice) and rebuild."""
...
ids=json.loads(eut("esearch.fcgi",db="pubmed",term=f'"{TITLE}"[Title]',retmode="json",retmax=6)).get("esearchresult",{}).get("idlist",[])
if not ids:
    ids=json.loads(eut("esearch.fcgi",db="pubmed",term="cBio Cancer Genomics Portal open platform multidimensional cancer genomics data",
                       retmode="json",retmax=8)).get("esearchresult",{}).get("idlist",[])
if not ids:
    ids=json.loads(eut("esearch.fcgi",db="pubmed",term="Cerami cBioPortal Cancer Discovery 2012",retmode="json",retmax=8)).get("esearchresult",{}).get("idlist",[])
print("candidate records:",ids)
js=json.loads(eut("esummary.fcgi",db="pubmed",id=",".join(ids),retmode="json") or "{}")
best=None
for uid,rec in (js.get("result") or {}).items():
    if uid=="uids": continue
    t=(rec.get("title") or "")
    if "correction" in t.lower() or "erratum" in t.lower(): continue
    if TITLE.lower()[:60] in t.lower():
        a=rec.get("authors") or []
        doi=""
        for x in rec.get("articleids") or []:
            if x.get("idtype")=="doi": doi=x.get("value","")
        best=dict(first_author=(a[0]["name"] if a else "Cerami E"), n_authors=len(a),
                  title=t.strip().rstrip("."), journal=rec.get("source") or "Cancer Discovery",
                  year=(rec.get("pubdate") or "")[:4], volume=rec.get("volume") or "",
                  pages=rec.get("pages") or "", doi=doi, pmid=uid)
        break
assert best, "could not resolve the cBioPortal paper"
print("resolved:", best["first_author"], best["year"], best["journal"], "|", best["title"][:60], "| doi:", best["doi"], "| pmid:", best["pmid"])
entry=f"{best['first_author']} {best['title']}. {best['journal']} {best['year']}"
if best["volume"]: entry+=f";{best['volume']}"
if best["pages"]: entry+=f":{best['pages']}"
entry+=f". doi:{best['doi']}" if best["doi"] else f". PMID:{best['pmid']}."
...
fixed=0; new=[]
for n,t in entries:
    if "Unknown author Correction" in t or t.lower().startswith("unknown author correction"):
        new.append((n,entry)); fixed+=1
    else: new.append((n,t))
block="**References**\n"+"\n".join(f"{n}. {t}" for n,t in new)+"\n\n"
md=head+block+tailsec.lstrip("\n")
open(mdp,"w",encoding="utf-8",newline="\n").write(md)
print("entries repaired:",fixed,"| references:",len(new))
# rebuild docx/pdf
...
print("docx superscripts:",sum(1 for p in d.paragraphs for r in p.runs if r.font.superscript),
      "| figures ok:",f"{sum(1 for h in em if h in disk.values())}/{len(em)}",
      "| 'Unknown author' left:",dt.count("Unknown author"))
z=os.path.join(r"C:\Users\fengq\Desktop","★Zenodo上传-就选这个_v14"); shutil.make_archive(z,"zip",PK)
stage=os.path.join(V14,"_stage9"); shutil.rmtree(stage,ignore_errors=True)
...
"Zenodo archive; the DOI is inserted in the manuscript once the deposit is published.\n")
...
for cmd in [["git","add","-A"],["git","-c","user.name=sushuqiong","-c","user.email=fengqi9666@qq.com","commit","-q","-m",
            "v14.4: repair the cBioPortal reference (was a correction notice) - 52 references, all with in-text numbers and DOIs"],
